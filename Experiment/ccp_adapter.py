...
# CCP adapter
# =========================
def _make_key_ccp(instance: str, beta: float, alpha: float, run_id: int):
    return (str(instance), round(float(beta), 10), round(float(alpha), 10), int(run_id))
...
def _build_summary_ccp(df: pd.DataFrame):
    if df.empty:
        return pd.DataFrame()
    ...
    summary = df.groupby(["instance", "beta", "alpha"]).agg(agg_dict)
    summary.columns = ["_".join(col).strip() for col in summary.columns.to_flat_index()]
    summary = summary.reset_index()
    return summary


def _make_archive_rows_ccp(rec: dict):
...
def _save_run_details_pkl_ccp(details_dir, instance_name, beta, alpha, run_id, detail_rec):
...
def _make_tasks_ccp(instance_list, BETAS, ALPHAS, N_RUNS, base_seed, done):
    tasks = []
    total_tasks = 0
    skipped = 0
        ...
        for beta in BETAS:
            for alpha in ALPHAS:
                for run_id in range(1, N_RUNS + 1):
                    total_tasks += 1
                    key = _make_key_ccp(instance_name, beta, alpha, run_id)
                    if key in done:
                        skipped += 1
                        continue
                    ...
                    tasks.append((instance_path, instance_name, float(beta), float(alpha), int(run_id), int(seed)))
