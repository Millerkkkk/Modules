...
from ..gb import GranularBallMedoid
...
class KMedoidsCluster:
    def __init__(
        ...
        self.gbs: List[GranularBallMedoid] = []

    def _kmedoids_2(self, pts: np.ndarray, max_iter: int = 30) -> np.ndarray:
        n = pts.shape[0]
        if n <= 1:
        ...
        i0 = int(np.argmax(D[i1]))
        medoids = np.array([i0, i1], dtype=int)
        ...
        for _ in range(max_iter):
            ...
            if not np.any(new_labels == 0) or not np.any(new_labels == 1):
                medoids = rng.choice(n, size=2, replace=False)
                continue
            ...
            for k in (0, 1):
                ...
                new_medoids[k] = int(idx[np.argmin(subD.sum(axis=1))])
    ...
    def _split_gb(self, gb: GranularBallMedoid) -> List[List[int]]:
        """
    ...
    def generate_gbs(self):
        ...
        while True:
            ...
            for gb_idx in gbs_list:
                if len(gb_idx) < self.threshold:
                    ...
                    # 注意：这里用临时 GranularBallMedoid 只是为了复用 _split_gb
                    tmp_gb = GranularBallMedoid(gb_idx, self.points)
        ...
        # ✅ 最终统一构造成 GranularBallMedoid 对象
        self.gbs = [GranularBallMedoid(sorted(set(idx)), self.points) for idx in gbs_list]
    ...
    def merge_single_point_gb(self) -> None:
        ...
        self.gbs = [GranularBallMedoid(sorted(set(idx)), self.points) for idx in new_idxs]
        
    def _merge_contained_gbs(self, merged_gbs):
        """
        merged_gbs: Dict[int, GranularBallMedoid]
        return: Dict[int, GranularBallMedoid]  (after containment merge)
        """
        # label -> (center, radius)
        ...
        out = {lab: GranularBallMedoid(sorted(set(idx)), self.points) for lab, idx in new_merged.items()}
        return out
    ...
    def merge_gbs(self):
        ...
        merged_gbs = {lab: GranularBallMedoid(sorted(set(idx)), self.points) for lab, idx in merged_idxs.items()}
...
def cluster_gb_kmedoids(
    ...
    *,
    ...
    model = KMedoidsCluster(pts, split_k=split_k, random_state=random_state)
