...
from CustomerPlanning_GA import plan_internal_gbs_order_ga, plan_customers_order, RouteContext
...
from plot import (
    plot_clusters_gbs,
    plot_clusters_gbs_with_gb_routes,
    plot_routes,
    plot_pareto_front,
    plot_search_evolution,
)

from VNS.neighborhood_no_gb import NeighborhoodNoGB
...
NS = [
    ...
    "relocate_node_intra_route",
]

NL = [
    "relocate_node_intra_route",
    "reverse_node_intra_route",
    "swap_node_intra_route",
    "relocate_node_inter_route",
    "relocate_boundary",
]
...
def freeze_routes(routes_by_clusters):
    ...
    Compatible with:
    1) GB version:   {cid: [[...], [...], ...]}
    2) NoGB version: {cid: [node1, node2, ...]}
    """
    frozen = []

    for cid, cluster_routes in sorted(routes_by_clusters.items(), key=lambda x: x[0]):
        if len(cluster_routes) == 0:
            frozen_cluster = tuple()
        elif isinstance(cluster_routes[0], int):
            # NoGB flat route
            frozen_cluster = tuple(cluster_routes)
        else:
            # GB nested structure
...
def run_once_spr(
    ...
    for cid, cluster in clusters.items():
        ...
        routes = plan_customers_order(
            problem,
            customer_ids,
            context=RouteContext(depot_mode="depart & return"),
            params=params_customer,
        )

        routes_clusters[cid] = routes.order
    ...
    # routes_clusters = {cid: [node for gb in gbs for node in gb] for cid, gbs in routes_clusters.items()}
    # print('routes_clusters', routes_clusters)
        ...
        for beta in betas:
            for run_id in range(1, n_runs + 1):
                ...
                if key in done:
                    skipped += 1
                    continue
                seed = int(base_seed + 1 * round(float(beta) * 10) + (run_id - 1))
                # seed = int(base_seed + 1000 * round(float(beta) * 10) + (run_id - 1))
